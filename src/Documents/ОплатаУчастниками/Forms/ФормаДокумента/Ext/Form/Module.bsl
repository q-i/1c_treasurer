...
Процедура ЗаполнитьВсеДолгиУчастникаНаСервере(ВыбранныйУчастник)
	...
	|ИЗ
	|	РегистрНакопления.ЗадолженностьУчастников.Остатки(&Период, Участник = &ВыбранныйУчастник) КАК ОстаткиЗУ
	|ГДЕ
	...
	|	Мероприятие
	|АВТОУПОРЯДОЧИВАНИЕ";
	Запрос.УстановитьПараметр("Период", ?(ЗначениеЗаполнено(Объект.Ссылка), Объект.Ссылка.МоментВремени(), Объект.Дата));
